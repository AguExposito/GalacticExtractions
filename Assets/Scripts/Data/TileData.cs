...
public class TileData:OreData
{
    ...
    bool AssignBiomeAndOreType() {
        ...
        switch (biomeGroundTile.name) {
            ...
            default: {
                    biomeResources = biomeResources.transform.GetChild(0).gameObject;
                    Debug.LogWarning("SE AUTOASIGNO BIOMA A GROUND, REVISAR SI FALTA A�ADIR UN BIOMA AL SWITCH"); 
                    biome = BiomeType.ground; 
                } break;
        }        
    ...
    {
        if (extractedOres >= oreAmount || health <= 0 || type == TileType.wall) return 0; // Ya est� agotado
