...
public class OreGenerator : MonoBehaviour
{
    ...
    void GenerateOres()
    {
        ...
        for (int i = 0; i < numClusters; i++)
        {
            ...
            if (!foundValidTile)
            {
                ...
                continue; // Saltar esta iteraci�n si no se encontr� un tile v�lido
            }
    ...
    void CreateOreCluster(Vector3Int start, TileBase oreType)
    {
        ...
        while (openSet.Count > 0 && closedSet.Count < clusterSize)
        {
            ...
            {
                // Solo expande si es un tile v�lido y no ha sido visitado
                if (!closedSet.Contains(neighbor) && mapGenerator.wallsTilemap.HasTile(neighbor))
                {
                    if (Random.value > 0.3f) // Probabilidad de expansi�n
                    {
